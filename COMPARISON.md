...
| Normal-Space front order | A normal-level panel can return behind other windows after leaving and re-entering its desktop | Observes active-Space changes and restores a visible normal-level panel to the front only when it returns to its own Space | The panel keeps its expected front order without enabling always-on-top or all-Spaces behavior |
...
| Window dragging | Dragging inside the WebView generally moves the entire window | Text pixels retain selection/copy priority; a 0.45-second hold on a blank pixel followed by a drag moves the window | Text can be selected and copied while blank space in either pane remains draggable |
...
| Shortcut configuration | Keyboard shortcuts are fixed in code | Adds an editable native shortcut-settings window with duplicate detection, persistence, and a restore-defaults command | Users can customize all listed shortcuts while retaining the original defaults |
